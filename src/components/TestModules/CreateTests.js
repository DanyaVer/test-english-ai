...
export default function CreateTests(props) {
  ...
  const { headings, headingAreLoading, generateHeading } = useHeadingGenerator(
    props.text
  );
  ...
  return (
    <>
      <CreateTestingModule
        ...
        headingAreLoading={headingAreLoading}
        statementsAreLoading={statementsAreLoading}
      />
      ...
      {submitted && (
        <ShowResults
          resultHeadings={{ correct: resultHeadings, amount: headings.length }}
          resultStatements={{
            correct: resultStatements,
            amount: statements.length,
          }}
        />
      )}
    </>
